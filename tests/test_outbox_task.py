...
def test_run_outbox_loop_builds_usecase_and_executes_once():
    async def run():
        usecase = AsyncMock()
        ...
        with patch("src.application.service.outbox_worker.asyncio.sleep", fast_sleep):
            try:
                await run_outbox_loop(build_usecase)
            except SystemExit:
                pass
